# -*- coding: utf-8 -*-
"""
Created on Fri Sep 12 22:43:41 2025
...
CATALOG_FLUX_MATCH_TOLERANCE = 5.0
...
def extract_reference_stars(fits_file, df_B, df_V,
    ...
    """Query Vizier and return reference stars with catalog mags and measured fluxes (B,V)."""
    ...
    # attach measured fluxes
    flux_B, flux_V = [], []
    for _, row in df_ref.iterrows():
        dB = np.sqrt((df_B["X"] - row["X_pix"])**2 + (df_B["Y"] - row["Y_pix"])**2)
        dV = np.sqrt((df_V["X"] - row["X_pix"])**2 + (df_V["Y"] - row["Y_pix"])**2)

        fB = df_B.loc[dB.idxmin(), "Flux"] if dB.min() <= flux_tol else np.nan
        fV = df_V.loc[dV.idxmin(), "Flux"] if dV.min() <= flux_tol else np.nan
...
def compute_zero_point(fluxes, mags):
    """Compute zero point given fluxes and catalog magnitudes."""
    return np.mean(mags + 2.5 * np.log10(fluxes))
